...
fn validate_fixture_set() {
    let fixtures_dir = common::repo_root()
        .join("schema")
        .join("fixtures")
        .join("aibom-v0.1.0");
    let expected_fixture_count = fixture_manifest_count(&fixtures_dir);
    assert!(
        expected_fixture_count > 0,
        "fixture count must be nonzero for {}",
    ...
    Command::cargo_bin("aibom-cli")
        ...
        .stdout(contains(format!(
            "{expected_fixture_count} fixtures checked, 0 failures"
        )));
}

fn fixture_manifest_count(fixtures_dir: &Path) -> usize {
    fs::read_dir(fixtures_dir)
        .unwrap()
        .filter_map(Result::ok)
        .map(|entry| entry.path())
        .filter(|path| path.is_dir())
        .flat_map(|stage_dir| fs::read_dir(stage_dir).unwrap())
        .filter_map(Result::ok)
...
#[test]
fn parent_of_homes_scan_redacts_usernames_in_aibom_and_report() {
    // Admin scanning a shared-home parent (/Users, C:\Users, /home) must not
    // leak local usernames into the AIBOM or the rendered HTML report. The
    // home-directory segment under a recognized home root is redacted to
    // <redacted-home>. Regression for the username-leak class.
    let target = TempDir::new().unwrap();
    let out = TempDir::new().unwrap();
    let users = target.path().join("Users");
    let rel = Path::new("Library")
        .join("Application Support")
        .join("Claude")
        .join("claude_desktop_config.json");
    for (home, server) in [("alice", "github"), ("bob", "slack")] {
        let cfg = users.join(home).join(&rel);
        fs::create_dir_all(cfg.parent().unwrap()).unwrap();
        fs::write(
            &cfg,
            format!(r#"{{"mcpServers":{{"{server}":{{"command":"uvx","args":["{server}"]}}}}}}"#),
        )
        .unwrap();
    }
    ...
    let aibom_path = fs::read_dir(out.path())
        .unwrap()
        .filter_map(Result::ok)
        .map(|entry| entry.path())
        .find(|p| {
            p.file_name()
                .and_then(|n| n.to_str())
                .is_some_and(|n| n.ends_with(".aibom.json"))
        })
        .expect("scan wrote an AIBOM sidecar");

    // Both child-home servers land in the one endpoint inventory.
    let aibom = common::read_json(&aibom_path);
    assert_eq!(
        aibom["aibom"]["components"].as_array().unwrap().len(),
        2,
        "both child-home servers should be discovered"
    );

    let aibom_text = fs::read_to_string(&aibom_path).unwrap();
    assert!(
        !aibom_text.contains("alice") && !aibom_text.contains("bob"),
        "AIBOM leaked a local username"
    );

    let report = out.path().join("redaction-report.html");
    Command::cargo_bin("aibom-cli")
        .unwrap()
        .args([
            "report",
            "--aibom",
            aibom_path.to_str().unwrap(),
            "--format",
            "html",
            "--output",
    ...
    assert!(
        ...
        "HTML report leaked a local username"
    );
}
...
fn fleet_manifest_indexes_and_fixture_signs_endpoint_artifacts() {
    ...
    assert_eq!(manifest["kind"], "reeve-fleet-manifest");
