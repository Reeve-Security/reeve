...
#   4. validate the 37-fixture contract-test corpus
...
# cosign dependency. Live Sigstore/Rekor acceptance is a separate release
# gate tracked by issue #13.
#
...
FIXTURES_DIR="${REPO_ROOT}/schema/examples/fixtures"
...
log "step 4/5: validate fixture corpus (${FIXTURES_DIR})"
"${AIBOM_BIN}" validate "${FIXTURES_DIR}" --schema "${SCHEMA}" \
  | tee "${WORK_DIR}/validate-corpus.stdout" \
  | grep -q "^41 fixtures checked, 0 failures$" \
  || fail "fixture corpus did not report 41 fixtures / 0 failures"
