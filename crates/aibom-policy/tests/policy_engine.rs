...
fn fixture_rule_a_emits_deny() {
    ...
    let aibom = read_json(repo_root().join(
        "schema/examples/fixtures/positive/03-undeclared-egress-delta/fixture-03.aibom.json",
    ));
    let cdx =
        read_json(repo_root().join(
            "schema/examples/fixtures/positive/03-undeclared-egress-delta/fixture-03.cdx.json",
        ));
...
fn fixture_unknown_extension_warns() {
    ...
    let aibom =
        read_json(repo_root().join(
            "schema/examples/fixtures/positive/08-reverse-dns-extension/fixture-08.aibom.json",
        ));
    let cdx = read_json(
        repo_root()
            .join("schema/examples/fixtures/positive/08-reverse-dns-extension/fixture-08.cdx.json"),
    );
    let verdicts = runtime
        .block_on(evaluate(
            &aibom,
...
fn v0_3_windows_profile_write_fixture_warns_risky_grant() {
    let runtime = tokio::runtime::Runtime::new().unwrap();
    let aibom = read_json(repo_root().join(
        "schema/examples/fixtures-v0.3.0/policy/03-policy-12-windows-profile-write-warn/fixture-03.aibom.json",
    ));
    let verdicts = runtime
        .block_on(evaluate(
            &aibom,
...
fn policy_depth_contract_fixtures_emit_expected_verdicts() {
    ...
    for (dir, scan_id, policy_id, config, signature) in cases {
        let fixture_dir = repo_root().join(format!("schema/examples/fixtures/policy/{dir}"));
