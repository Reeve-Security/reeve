...
fn fixture_paths(kind: &str) -> Vec<PathBuf> {
    let root = repo_root()
        .join("schema/examples/sensitive-data-report")
        .join(kind);
...
fn positive_secret_rule_pack_fixtures_validate() {
    ...
    let root = repo_root().join("schema/examples/secret-rule-pack/positive");
    let mut fixture_count = 0;
...
fn negative_secret_rule_pack_fixtures_reject() {
    let validator = secret_rule_pack_validator();
    let root = repo_root().join("schema/examples/secret-rule-pack/negative");
    let mut fixture_count = 0;
