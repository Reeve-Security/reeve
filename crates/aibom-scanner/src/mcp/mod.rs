...
/// an OS username. The absolute path shape is retained per ; matching
...
/// [`redact_encoded_project_segment`]. See  (#468 extends to
...
#[cfg(test)]
mod redaction_tests {
    use super::redact_home_identity;

    #[test]
    fn redacts_home_segment_under_recognized_roots() {
        assert_eq!(
            redact_home_identity("/Users/alice/Library/x"),
            "/Users/<redacted-home>/Library/x"
        );
        assert_eq!(
            redact_home_identity("/home/alice/.config/x"),
            "/home/<redacted-home>/.config/x"
        );
        // Windows root is case-insensitive with backslash separators (the
        // class that previously leaked a username).
        assert_eq!(
            redact_home_identity(r"C:\Users\Alice\AppData\x"),
            r"C:\Users\<redacted-home>\AppData\x"
        );
    }

    #[test]
    fn non_home_root_parent_is_left_intact() {
        // Boundary: only `Users`/`home` segments trigger redaction. A parent
        // that is not a recognized home root is echoed as-is.
        let p = "/var/data/root/alice/Library/x";
        assert_eq!(redact_home_identity(p), p);
    }
}
