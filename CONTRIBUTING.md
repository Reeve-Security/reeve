...
Install the repository hook path before you start making changes:

```bash
git config core.hooksPath .githooks
```

The pre-commit hook runs `gitleaks` against staged changes with the
repository config in [`.gitleaks.toml`](.gitleaks.toml). Install it with:
...
- `tools/` is reusable open-source machinery: deploy helpers, MDM templates,
  validation labs, and other product-adjacent automation that is intended to
...
- Do not commit generated `*.aibom.json`, Terraform state, or ad-hoc
  `inventory.yml` files outside the tracked fixture corpus.
